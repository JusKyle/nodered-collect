...
interface DispatchConfirmBubbleProps {
  ...
  onDispatch?: (success: boolean, error?: string) => void
}

function DispatchConfirmBubble({ deviceInstance, onDispatch }: DispatchConfirmBubbleProps) {
  ...
  const gatewayName = deviceInstance.gateway?.name || deviceInstance.gatewayId
  ...
  const handleDispatch = async () => {
    ...
    try {
      ...
      await updateDeviceInstance(deviceInstance.id, { status: 'SYNCING' })
      setShowBubble(false)
      onDispatch?.(true)
    } catch (err: any) {
      const errorMessage = err.response?.data?.message || '下发失败'
      setError(errorMessage)
      onDispatch?.(false, errorMessage)
    } finally {
      setIsLoading(false)
    }
  }
