...
import { updateDeviceModelBasic } from '../../api/device-model.api'
...
function DeviceModelEditModal({ isOpen, onClose, model }: DeviceModelEditModalProps) {
  ...
  useEffect(() => {
    if (model && isOpen) {
      reset({
        modelDI: model.modelDI || model.model,
        name: model.name,
  ...
  const onSubmit = async (data: FormData) => {
    if (!model) return
    try {
      await updateDeviceModelBasic(model.id, {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h2 className="text-lg font-semibold text-gray-900">编辑设备模板</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">协议类型</label>
            <input
              {...register('protocol')}
              type="text"
              disabled
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" disabled={isSubmitting} className="px-6 py-2.5 bg-primary-500 text-white rounded-lg hover:bg-indigo-600 font-medium disabled:opacity-60 disabled:cursor-not-allowed">
              {isSubmitting ? '保存中...' : '保存'}
            </button>
          </div>
        </form>
