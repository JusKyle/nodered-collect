  ...
  const handleTestConnection = async () => {
    ...
    try {
      const result = await testConnection({
        ...
        port: parseInt((document.getElementById('port') as HTMLInputElement).value) || 1880,
        adminToken: (document.getElementById('adminToken') as HTMLInputElement).value,
      })
      if (result.success) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h2 className="text-lg font-semibold text-gray-900">新建网关</h2>
          <button
            onClick={handleClose}
