...
- [ ] DeviceModel 表有 version 字段（Int，默认 1）
- [ ] PointModel 表存在，modelId 外键关联 DeviceModel
- [ ] PointModel 的 config 字段是 JSON 类型
- [ ] dataType 是枚举类型，包含 INT16/UINT16/INT32/UINT32/FLOAT32/FLOAT64/BOOL/STRING
- [ ] 可以插入一条 DeviceModel 和若干 PointModel 记录
...
**验证标准**：
- [ ] GET /api/device-models → 返回 list + total
- [ ] ?name=温度 → 只返回名称含"温度"的模板
- [ ] ?protocol=MODBUS_TCP → 只返回 Modbus TCP 的模板
- [ ] ?page=2&pageSize=10 → 返回第 2 页
- [ ] 每条记录包含 pointCount（点位数量）
- [ ] 默认按 updatedAt 倒序
...
**验证标准**：
- [ ] POST 传入正确参数 → 返回 201，data 含 id 和 version=1
- [ ] 数据库中新增一条记录，version = 1
- [ ] 不传 name → 返回 400
- [ ] 不传 modelDI → 返回 400
- [ ] modelDI 重复 → 返回 409
- [ ] protocol 不是枚举值 → 返回 400
...
**验证标准**：
- [ ] 点新增按钮 → 弹窗出现
- [ ] 表单为空时点确定 → 各字段下方显示红色错误提示
- [ ] 选了协议、填了名称和型号 → 提交成功，弹窗关闭
- [ ] modelDI 填中文 → 提示格式错误
...
**验证标准**：
- [ ] GET /api/device-models/:id → 返回完整详情 + points 数组
- [ ] points 按 sort 升序排列
- [ ] 不存在的 id → 返回 404
- [ ] 返回 version 字段
...
**验证标准**：
- [ ] PUT /api/device-models/:id/basic 传入 name 新值 → 名称更新，version 不变
- [ ] 传 description → 备注更新
- [ ] 试图修改 protocol → 忽略，不报错
- [ ] modelDI 和其他模板重复 → 返回 409
- [ ] 不存在的 id → 返回 404
...
**验证标准**：
- [ ] GET /api/device-models/:id/points → 返回 list + total
- [ ] 有 mock 数据时，返回的点位字段完整
- [ ] ?name=温度 → 只返回名称含"温度"的点位
- [ ] 按 sort 升序排列
- [ ] 模板不存在 → 404
...
**验证标准**：
- [ ] POST 新建点位 → 返回 201，点位创建成功
- [ ] 创建后模板 version 比原来大 1
- [ ] 同模板 tag 重复 → 返回 409
- [ ] 不传 name → 返回 400
- [ ] 模板不存在 → 404
...
**验证标准**：
- [ ] PUT 修改点位名称 → 名称更新，模板 version + 1
- [ ] 修改 config → 配置更新，version + 1
- [ ] tag 改成和其他点位重复 → 409
- [ ] 点位不存在 → 404
- [ ] 只改一个字段，其他字段保持不变
...
**验证标准**：
- [ ] DELETE 删除点位 → 数据库查不到该点位
- [ ] 删除后模板 version + 1
- [ ] 点位不存在 → 404
- [ ] 删除后，后面的点位 sort 值补上（保持连续）
...
**验证标准**：
- [ ] 点新增点位 → 弹窗出现
- [ ] Modbus TCP 协议的模板 → 显示站号、寄存器类型、起始地址等字段
- [ ] S7 协议的模板 → 显示 DB 号、偏移量等字段
- [ ] 必填项为空 → 提交时报错
- [ ] 填写正确 → 提交成功，列表多了一条，版本号 +1
- [ ] 点位标识重复 → 提示错误
...
**验证标准**：
- [ ] 上传正确的 Excel → 返回成功数量，点位批量创建，版本 +1
- [ ] 上传有一行错误的 Excel → 返回错误详情（行号+原因），不导入任何数据
- [ ] 上传空文件 → 返回错误
- [ ] 上传不支持的格式 → 返回错误
- [ ] 有重复 tag 的 → 校验失败
...
**验证标准**：
- [ ] GET /api/device-models/:id/points/export → 返回文件流
...
**验证标准**：
- [ ] 点导出按钮 → 浏览器下载 Excel 文件
- [ ] 点导入按钮 → 弹窗出现
- [ ] 拖拽 Excel 文件到弹窗 → 上传中状态
- [ ] 上传成功 → 提示成功 N 条，关闭后列表刷新
- [ ] 上传 txt 文件 → 前端提示格式不支持
- [ ] 上传有错误的 Excel → 弹窗显示错误列表
...
**验证标准**：
- [ ] DELETE 删除 → 模板和点位都从数据库消失
- [ ] 模板不存在 → 404
- [ ] 有实例关联时 → 返回 400，提示有设备实例使用
- [ ] 没有关联时删除成功
...
**验证标准**：
- [ ] 正确的 Modbus TCP 配置 → 校验通过
- [ ] slaveId = 0 → 校验失败，提示 1-247
- [ ] slaveId = 248 → 校验失败
- [ ] registerType = Holding, dataType = FLOAT32 → 通过
- [ ] registerType = Coil, dataType = FLOAT32 → 校验失败，线圈不支持浮点
- [ ] startAddress = 40001 → 校验通过
...
**验证标准**：
- [ ] 正确的 RTU 配置 → 校验通过
- [ ] baudRate = 1234 → 校验失败，不在枚举值
- [ ] dataBits = 7, parity = None → 校验失败，7位数据位需要校验
- [ ] stopBits = 1, baudRate = 9600 → 通过
- [ ] 寄存器配置部分和 TCP 一样
...
**验证标准**：
- [ ] area = DB, dbNumber = 1, byteOffset = 0, dataType = INT16 → 通过
- [ ] dataType = BOOL, 没有 bitOffset → 校验失败
- [ ] dataType = BOOL, bitOffset = 3 → 通过
- [ ] area = I, dataType = INT16 → 通过（I区不需要DB号）
- [ ] dbNumber = 65536 → 校验失败，超出范围
...
**验证标准**：
- [ ] nodeId = "ns=2;s=Temperature" → 校验通过，namespaceIndex = 2
- [ ] nodeId = "ns=3;i=1001" → 通过
- [ ] nodeId = "invalid" → 校验失败，格式不正确
- [ ] nodeId = "ns=0;s=Server" → 通过
- [ ] browseName 可选，填不填都可以
...
**验证标准**：
- [ ] topic = "device/+/data", jsonPath = "$.temp" → 通过
- [ ] topic = "sensor/#" → 通过
- [ ] 空 topic → 校验失败
- [ ] jsonPath = "$.data[0].value" → 通过
- [ ] qos = 3 → 校验失败，只能 0/1/2
...
**验证标准**：
- [ ] requestHex = "010300000002C40B" → 通过
- [ ] requestHex = "ZZZZ" → 校验失败，不是合法十六进制
- [ ] responseOffset = 3, byteOrder = Big-Endian → 通过
- [ ] responseOffset = -1 → 校验失败
- [ ] 空 requestHex → 校验失败
...
**验证标准**：
- [ ] Modbus TCP 模板新增点位 → 显示站号、寄存器类型、起始地址等字段
- [ ] S7 模板新增点位 → 显示区域、DB号、字节偏移、位偏移
- [ ] OPC UA 模板新增点位 → 显示 NodeId、命名空间
- [ ] MQTT 模板新增点位 → 显示主题、JSON路径、QoS
- [ ] 每个表单的必填校验正常工作
- [ ] 编辑时能正确回填协议配置数据
- [ ] 切换不同协议的模板 → 配置表单跟着变
...
- [ ] 所有 30 个任务的验证标准（测试用例）全部通过
- [ ] AC 覆盖总表中所有 AC 的验证方式已执行并通过
- [ ] 数据库 migration 脚本在测试环境验证通过
- [ ] 前端 2 个页面（列表、详情）+ 7 个弹窗组件均可正常操作
- [ ] 6 种协议的点位配置（Modbus TCP/RTU、S7、OPC UA、MQTT、TCP）全部支持
- [ ] 点位增删改后版本号正确递增，编辑基本信息不递增
- [ ] 点位导入导出功能正常，Excel 格式正确
- [ ] 模板复制功能验证通过
